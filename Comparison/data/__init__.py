...
mnist_to_reduce = torchvision.datasets.MNIST(root = str(_DATA_ROOT), train = True, transform = transform)

datasets = {
    "100.0": torchvision.datasets.MNIST(
...
def add_reduce_dataset(size, name):
    if size != 100:
        size = size/100
        ds = random_split(mnist_to_reduce, [size, 1-size], generator=torch.Generator().manual_seed(42))
        datasets[name] = ds[0]
...
MNIST_train = MNIST_Images("100.0")
